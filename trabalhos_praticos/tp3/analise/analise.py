...
# Adicione esta linha para definir a extensão do executável
...
def compile_code():
    ...
    try:
        ...
        result = subprocess.run(
            ["gcc", "-O2", "-o", f"genwkl3{EXE_EXT}", gen_path, "-lm"], # Modificado
        ...
        command = [
            "g++", "-std=c++11", "-O2", "-Wall",
            "-I./include",
            "-o", f"simulador{EXE_EXT}" # Modificado
        ] + source_files
...
def generate_input_file(nodes, packets, clients, seed):
    ...
    try:
        ...
        subprocess.run(
            [f"./genwkl3{EXE_EXT}", "-s", str(seed), "-n", str(nodes), "-p", str(packets), "-c", str(clients), "-t", "10"], # Modificado
            check=True, capture_output=True, text=True
        )
...
def run_simulation(input_file):
    ...
    simulador_path = f"./simulador{EXE_EXT}" # Modificado
    if not (os.path.exists(simulador_path) and os.path.exists(input_file)): # Modificado
        return None
    try:
        start_time = time.time()
        result = subprocess.run(
            [simulador_path, input_file], # Modificado
...
def main():
    ...
    files_to_clean = [f"genwkl3{EXE_EXT}", f"simulador{EXE_EXT}", "tp3.in"] # Modificado
